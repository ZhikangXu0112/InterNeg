...
class CLIP_scoring(nn.Module):
    def __init__(self, clip_model, tokenized_prompts,class_img):
        super().__init__()
    ...
    def prepare_id(self, tokenized_prompts_id):
        ...
        cos_sim = torch.sum(torch.mul(text_features,self.class_img),dim=1)  #(1000,1)
        return cos_sim
    ...
    def forward(self, tokenized_prompts_wordnet):
        with torch.no_grad():
            ...
            cos_sim = text_features_wordnet @ self.class_img.t()
            cos_sim2 = text_features_wordnet @ self.text_features.t()

        return cos_sim,cos_sim2
...
def generate_cossim_idname_wordnet_dedup(classnames, save_path, class_img, model):
    ...
    # class_num=1000
    # img_path=path
    clip_selector = CLIP_scoring(model, tokenized_prompts, class_img)
    ...
    for i in range(total_iter):
        ...
        cos_sim,cos_sim2 = clip_selector(current_token) ## batch * 1000
        can_list_adj += current_list
        can_cos_adj.append(cos_sim)
        can_cos_adj2.append(cos_sim2)
    can_cos_adj = torch.cat(can_cos_adj, dim=0)
    can_cos_adj2 = torch.cat(can_cos_adj2, dim=0)
