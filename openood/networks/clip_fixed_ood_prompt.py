...
dtd_split_classes = ['banded', 'blotchy', 'braided', 'bubbly', 'bumpy', 'chequered', 'cobwebbed', 'cracked', 'crosshatched', 'crystalline', 'dotted', 'fibrous', 'flecked', 'freckled', 'frilly', 'gauzy', 'grid', 'grooved', 'honeycombed', 'interlaced', 'knitted', 'lacelike', 'lined']
...
ninco_classes = ["amphiuma_means","s_sky","sarracenia_alata","triturus_marmoratus","araneus_gemma","skipper_caterpillar","pyramid","sepia_apama","erysimum_franciscanum","lepomis_auritus","door","windsor_chair","f_forest_path","french_fries","lasionycteris_noctivagans","Caracal_caracal_caracal","waffles","microcystis_wesenbergii","gramophone","octopus_bimaculoides","ctenolepisma_longicaudata","batrachoseps_attenuatus","streptopus_lanceolatus","sarpa_salpa","chicken_quesadilla","bagpipe","haemulon_sciurus","walker","ferocactus_pilosus","sepia_officinalis","epithelantha_micromeris","fireworks","tapirus_bairdii","arctocephalus_galapagoensis","tursiops_aduncus","vaccinium_reticulatum","donuts","spaghetti_bolognese","f_field_road","cirsium_pitcheri","hindu_temple","mbira","leptoglossus_phyllopus","empty_water_dispencer","hippopus_hippopus","fire_extinguisher","sepioteuthis_australis","shuttlecock","dendrolagus_lumholtzi","creme_brulee","polistes_dominula","octopus_rubescens","cable","platycephalus_fuscus","stapler","scissors","high_heels","darlingtonia_californica","aphanizomenon_flosaquae","cup_cakes","glass_of_milk","pseudorca_crassidens","lathyrus_odoratus","ozotoceros_bezoarticus"]
...
def get_class_names(dataset):
    if dataset == 'imagenet':
        ...
        return dtd_classes
    elif dataset == 'dtd_split':
        return dtd_split_classes
    elif dataset == 'eurosat':
        return eurosat_classes
    elif dataset == 'fgvc_aircraft':
        return fgvc_aircraft_classes
    elif dataset == 'food101':
        return food101_classes
    elif dataset == 'ninco':
        return ninco_classes
    elif dataset == 'sun':
        return sun397_classes
    else:
...
def get_selected_ood_text_list(model, dataset='imagenet', total_ood_num=1000, class_img=None, backbone=None):
    print('the ID dataset is:', dataset)
    print('total_ood_num is:', total_ood_num)
    #######################################
    foot_path = './data/txtfiles_output/'

    wordnet_processed_path = foot_path + backbone + '_wordnet_' + dataset + '_cossim_dedup.pth'
    if os.path.exists(wordnet_processed_path):
        wordnet_dict = torch.load(wordnet_processed_path)
    else:
        classnames = get_class_names(dataset)
        generate_cossim_idname_wordnet_dedup(classnames, wordnet_processed_path, class_img, model)
        wordnet_dict = torch.load(wordnet_processed_path)


    _can_list_adj = wordnet_dict['text_list_adj'] 
    can_cos_adj = wordnet_dict['cos_sim_adj']
    can_cos_adj2 = wordnet_dict['cos_sim_adj2']  # Neglabel
    _can_list_noun = wordnet_dict['text_list_noun'] 
    can_cos_noun = wordnet_dict['cos_sim_noun'] 
    can_cos_noun2 = wordnet_dict['cos_sim_noun2'] # Neglabel
    can_cos_id = wordnet_dict['cos_sim_id'] 
    _class_img = wordnet_dict['class_img'] 

    # print(can_cos_id.shape,can_cos_noun.shape,can_cos_adj.shape)  
    #torch.Size([1000]) torch.Size([58101, 1000]) torch.Size([11453, 1000])


    can_cos_id = can_cos_id.unsqueeze(0)
    mask = torch.all(can_cos_adj < can_cos_id, dim=1)
    selected_value_cos_adj = can_cos_adj[mask]
    can_list_adj = [s for s, m in zip(_can_list_adj, mask) if m]

    mask = torch.all(can_cos_noun < can_cos_id, dim=1)
    selected_value_cos_noun = can_cos_noun[mask] 
    can_list_noun = [s for s, m in zip(_can_list_noun, mask) if m]
    #torch.Size([7543, 1000]) torch.Size([53036, 1000])
    print('actual length of selected adj texts', len(can_list_adj))
    print('actual length of selected noun texts', len(can_list_noun))


    adj_num = int(total_ood_num * (len(can_list_adj) / (len(can_list_adj) + len(can_list_noun))))
    noun_num  = total_ood_num - adj_num

    value_cos_adj = (can_cos_id - selected_value_cos_adj).mean(dim=1) 
    value_cos_noun = (can_cos_id - selected_value_cos_noun).mean(dim=1)

    value_sim_adj, indice_sim_adj= value_cos_adj.sort(descending=True) ## larger to smaller
    value_sim_noun, indice_sim_noun = value_cos_noun.sort(descending=True) 

    
    #NegLabel
    _value_cos_adj2, _ = can_cos_adj2.sort(1) ## small cos sim to large cos sim.
    _value_cos_noun2, _ = can_cos_noun2.sort(1)
    if dataset=='imagenet':
        value_cos_adj2 = _value_cos_adj2[:, 950] ## larger means closer to ID
        value_cos_noun2 = _value_cos_noun2[:, 950]  
    elif dataset=='cifar10':
        value_cos_adj2 = _value_cos_adj2[:, 9] ## larger means closer to ID
        value_cos_noun2 = _value_cos_noun2[:, 9]  
    elif dataset=='cifar100':
        value_cos_adj2 = _value_cos_adj2[:, 95] ## larger means closer to ID
        value_cos_noun2 = _value_cos_noun2[:, 95]  
    elif dataset=='covid':
        value_cos_adj2 = _value_cos_adj2[:, 0] ## larger means closer to ID
        value_cos_noun2 = _value_cos_noun2[:, 0]  

    value_sim_adj2, indice_sim_adj2= value_cos_adj2.sort(0) ## smaller to larger, 13101
    value_sim_noun2, indice_sim_noun2 = value_cos_noun2.sort(0) ## 58723.
    ...
    adj_num2 = int(total_ood_num * (len(_can_list_adj) / (len(_can_list_adj) + len(_can_list_noun))))
    noun_num2  = total_ood_num - adj_num2

    selected_adj_text2 = [_can_list_adj[i] for i in indice_sim_adj2[:adj_num2]]
    selected_noun_text2 = [_can_list_noun[i] for i in indice_sim_noun2[:noun_num2]]
    ...
    # selected_adj_noun_path = foot_path + backbone + '_' + dataset + '_selected_adj_noun_'+ str(total_ood_num) + '_dedup.pth'
    # save_dict = {
    ...
    return selected_adj_text, selected_noun_text, _class_img.cuda(),selected_adj_text2, selected_noun_text2


# https://github.com/openai/CLIP/blob/main/notebooks/Prompt_Engineering_for_ImageNet.ipynb
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number, class_img, backbone, neglabel=False):
    classnames = get_class_names(dataset) # imagenet --> imagenet class names
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    ...
    # mean_text /= mean_text.norm()
    # mean_text = mean_text.view(-1, 1) 
    selected_adj_text_ours, selected_noun_text_ours,_class_img,selected_adj_text_neglabel, selected_noun_text_neglabel = get_selected_ood_text_list(model, dataset=dataset, total_ood_num=ood_number, class_img = class_img, backbone=backbone)
    if neglabel:
        selected_adj_text = selected_adj_text_neglabel
        selected_noun_text = selected_noun_text_neglabel
    else:
        selected_adj_text = selected_adj_text_ours
        selected_noun_text = selected_noun_text_ours
    ...
    return text_features_id_ood, text_features_unselected, _class_img, np.array(selected_adj_text+selected_noun_text)


class FixedCLIP_NegOODPrompt(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.backbone = cfg.backbone.name # 'ViT-B/16'
        classnames = get_class_names(cfg.backbone.dataset) # imagenet 
        self.n_cls = len(classnames)
        # pdb.set_trace()
        self.n_output = self.n_cls + cfg.backbone.ood_number
        assert self.backbone in clip.available_models()
        # clip_model, self.preprocess = clip.load(backbone, device='cuda')
        print(f"Loading CLIP (backbone: {self.backbone})")
        clip_model = load_clip_to_cpu(self.backbone)
        clip_model = clip_model.cuda()
        self.model = clip_model
        self.logit_scale = clip_model.logit_scale.data
        print("Turning off gradients in both the image and the text encoder")
        ...
        self.class_img = None
        self.text_features, self.text_features_unselected, self.class_img, self.ood_classnames = get_text_features_neg(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt, \
                                                        cfg.backbone.text_center, cfg.backbone.ood_number, self.class_img, self.backbone, True)
...
class FixedCLIP_InterNegOODPrompt(nn.Module):
    def __init__(self, cfg, id_loader_dict,ood_loader_dict):
        super().__init__()
        self.backbone = cfg.backbone.name # 'ViT-B/16'
        classnames = get_class_names(cfg.backbone.dataset) # imagenet 
        self.id_dataset = cfg.backbone.dataset
        self.n_cls = len(classnames)
        # pdb.set_trace()
        self.n_output = self.n_cls + cfg.backbone.ood_number
        assert self.backbone in clip.available_models()
        ...
        self.word_embed_dim = clip_model.token_embedding.weight.shape[1]
        self.logit_scale = clip_model.logit_scale.data
        print("Turning off gradients in both the image and the text encoder")
        for name, param in clip_model.named_parameters():
            param.requires_grad_(False) ## fix clip model.
        
        self.templates = get_templates('full')
        self.id_classnames = np.array(classnames)


        if not os.path.exists('./data/txtfiles_output/'+ self.backbone + '_wordnet_' + cfg.backbone.dataset + '_cossim_dedup.pth'):
            class_data = [None] * self.n_cls  
            class_count = [0] * self.n_cls
            ...
            with torch.no_grad():
                for batch in tqdm(id_loader_dict['train']):
                    ...
                    for i in range(len(label)):
                        class_idx = label[i]
                        sample = image_features[i]

                        if class_data[class_idx] is None:
                            class_data[class_idx] = sample  
                        else:
                            class_data[class_idx] += sample
                        
                        class_count[class_idx] += 1  
         
            class_img = torch.stack([class_data[i] / class_count[i] for i in range(self.n_cls)]).cuda()
            self.class_img = class_img / class_img.norm(dim=-1, keepdim=True)
        else:   
            self.class_img = None
        # self.ood_classnames for case study
        self.text_features, self.text_features_unselected, self.class_img,self.ood_classnames = get_text_features_neg(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt, \
                                                        cfg.backbone.text_center, cfg.backbone.ood_number, self.class_img, self.backbone)
