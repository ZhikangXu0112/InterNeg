    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        if self.setup_flag:
            ...
            with torch.no_grad():
                output_text = net.logit_scale * net.text_features_unselected.t() @ net.text_features # class_num * class_num
                output_text = torch.softmax(output_text, dim=1) 
            all_weights_text = output_text[:, :net.n_cls].sum(1) # prob of ID 
            self.text_idscore_unselected_cache = all_weights_text ## 11k
            ...
            with torch.no_grad():
                for batch in tqdm(id_loader_dict['train'],
                    ...
                    # weights = output_prob[indice, labels]
                    ########################## id/ood level sample weights
                    weights = output_prob[:, :net.n_cls].sum(1)
            ...
            all_feats_id = torch.cat(all_feats[:net.n_cls], dim=0) ## 11k * 512
            all_weights_id = torch.cat(all_weights[:net.n_cls], dim=0) ## 11k * 512
            # pdb.set_trace()
            all_feats_ood = torch.cat(all_feats[net.n_cls:], dim=0) ## 11k * 512
            all_weights_ood = torch.cat(all_weights[net.n_cls:], dim=0) ## 11k * 512
            self.image_classifier = all_feats_id
